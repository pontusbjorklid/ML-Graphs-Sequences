...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int,
        ...
        class_top = class_counts.argmax().item()
        p_A_lower_bound = lower_confidence_bound(class_top, num_samples, alpha)
        ##########################################################
        ...
        if p_A_lower_bound < 0.5:
            ...
            ##########################################################
            # YOUR CODE HERE
            rad = self.sigma * norm.ppf(p_A_lower_bound)
            return class_top, rad
            ##########################################################
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float,
        ...
        # YOUR CODE HERE
        class_top = class_counts.argmax().item()
        count1 = class_counts[class_top].item()
        count2 = num_samples - count1

        p_value = binom_test(count1, count1 + count2, p=0.5, alternative="greater")
        if p_value > alpha:
            return class_top
        else:
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int,
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                ##########################################################
                # YOUR CODE HERE
                predictions = self.base_classifier((batch + random_numbers).clamp(0, 1)).argmax(1)
                class_counts += torch.bincount(predictions, minlength=self.num_classes)
