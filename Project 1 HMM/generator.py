...
class HMM_TxtGenerator:
    ...
    def forwards(self, x):
        ...
        ### YOUR CODE HERE ###
        alpha = np.zeros((T, K))
        pi_squeezed = np.squeeze(pi)
        for t in range(T):
            if t == 0:
                alpha[t, :] = pi_squeezed * B[:, x[t]]
            else:
    ...
    def backwards(self, x):
        ...
        for t in range(T-1, -1, -1):
            if t == T-1:
                ...
                beta[t, :] = np.dot(A, B_x[:, t+1] * beta[t+1, :])
        return beta
    ...
    def generate_sentence(self, sentence_length):
        ...
        state_sequence = np.zeros(sentence_length, dtype=int)
        observation_sequence = np.zeros(sentence_length, dtype=int)

        state_sequence[0] = np.random.choice(K, p=np.squeeze(pi))

        observation_sequence[0] = np.random.choice(V, p=B[state_sequence[0], :])

        for t in range(1, sentence_length):
            state_sequence[t] = np.random.choice(K, p=A[state_sequence[t-1], :])
            observation_sequence[t] = np.random.choice(V, p=B[state_sequence[t], :])

        sent = self.X_to_sentence(observation_sequence)
